...
from elementary.clients.az.client import AZClient
...
class DataMonitoringReport(DataMonitoring):
    def __init__(
        ...
        self.az_client = AZClient.create_client(self.config, tracking=self.tracking)
    ...
    def send_report(
        ...
        if self.s3_client or self.gcs_client or self.az_client:
    ...
    def upload_report(
        ...
        if self.az_client:
            send_succeded, bucket_website_url = self.az_client.send_report(
                local_html_path, remote_bucket_file_path=remote_file_path
            )
            self.execution_properties["sent_to_az_successfully"] = send_succeded
            if not send_succeded:
                self.success = False        
